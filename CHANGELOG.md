...
- **减条数优先于截断**：`DigestFormatter` 拆出 `_build_body` + `_fit_sections`——整体超 600 字时先减条数（5 件事 5→4→3、企业节 5→4→3，均不低于 3 条）找到首个 ≤600 的组合，而非截断单条；`_fit` 仅作极端情况兜底。放宽单条上限（一句话 60 / 标题 40 / 企业动态 60 字）。
