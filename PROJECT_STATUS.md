...
- [x] v0.7.0a12：Windows 一键自动化部署（本地环境、全量质量门、GitHub 私有仓库、Actions、Secrets、首次验收）+ 可重复/无人值守参数 + 提交前密钥扫描；Manual Run/调度工作流改用环境变量和 Bash 参数数组，消除输入直接拼接；416 pytest + ruff + mypy 干净
