...
- **一键部署入口**：新增根目录 `一键部署.cmd` 与 `scripts/04_One-Click-Deploy.ps1`，串联隔离环境、依赖安装、配置校验、pytest/ruff/mypy、GitHub 授权、私有仓库创建/复用、推送、工作流启用、Secrets 和首次 Manual Run。
...
- **最小权限**：部署脚本保留仓库已有 Actions 白名单和默认令牌权限；工作流仅在自身声明所需的 `contents: write`。
...
- **背景**：用户反馈推送里"话说一半"——旧的分节硬截断（一句话≤50 / 5 件事每条≤30 / 企业节每条≤42 字）会把句子切在半句，且 600 字预算下 5 件事完整标题挤占企业节。要求：尽量完整、如果 5 条写不完就 3 条、条目控制在 3~5 之间。
- **语义截断**：`_truncate` 超长时优先截到句子结束符（。！？；!?;），截到完整句子不加省略号；仅当没有合适边界时才硬截并加省略号（结果严格 ≤ max_len）。
- **减条数优先于截断**：`DigestFormatter` 拆出 `_build_body` + `_fit_sections`——整体超 600 字时先减条数（5 件事 5→4→3、企业节 5→4→3，均不低于 3 条）找到首个 ≤600 的组合，而非截断单条；`_fit` 仅作极端情况兜底。单条上限取宽松值（一句话 60 / 标题 50 / 企业动态 90 字），保证正常完整句子（60~80 字）不被截半句。
- **报告链接相对路径**：`ReportEngine` 把报告链接从 GitHub runner 绝对路径（`/home/runner/work/...`，点不开且浪费 ~60 字预算）转为相对仓库根路径（`output/reports/<run_id>/report.md`，`_repo_relative` 兜底）。
- **测试**：语义截断边界 + 减条数不硬截（条数 3~5）+ `_fit` 兜底白盒；全量 pytest / ruff / mypy 干净。
